...
def parse_jsonl(filepath: str | Path) -> List[Dict[str, Any]]:
    ...
    with open(filepath, "rb") as f:
        ...
        results = list(
            toolz_filter(
                lambda x: x is not None, toolz_map(parse_line_safe, enumerate(f, 1))
            )
        )
    ...
    if error_details:
        logger.info(
            f"Parsed {len(messages)} messages, skipped {len(error_details)} malformed lines"
        )
...
def validate_claude_format(filepath: str | Path) -> Tuple[bool, List[str]]:
    ...
    def has_claude_signature(msg):
        return pipe(
            claude_signatures, lambda sigs: toolz_map(lambda sig: sig in msg, sigs), any
        )
    ...
    session_ids = set(
        toolz_filter(None, toolz_map(lambda msg: msg.get("sessionId"), messages))
    )
    ...
    errors = list(toolz_map(lambda x: x[1], toolz_filter(lambda x: x[0], error_checks)))
