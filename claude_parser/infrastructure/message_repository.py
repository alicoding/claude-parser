...
from toolz import (
    pipe, map as toolz_map, filter as toolz_filter,
    concat, partition, reduce, merge
)
...
class JsonlMessageRepository(MessageRepository):
    ...
    def load_messages(self, filepath: Path) -> List[Message]:
        ...
        """
        def parse_line_with_index(indexed_line: Tuple[int, bytes]) -> Optional[Dict[str, str]]:
            ...
            try:
                ...
                if message:
                    ...
                    if isinstance(message, list):
                        return pipe(
                            message,
                            lambda msgs: toolz_map(lambda m: {'type': 'message', 'data': m}, msgs),
                            list
                        )
                    else:
    ...
    def load_messages_streaming(self, filepath: Path):
        ...
        pass  # Implementation for future if needed