...
class BaseContentBlock(BaseModel):
    ...
    type: ContentBlockType = Field(description="Content block type")
...
class ThinkingContent(BaseContentBlock):
    ...
    content: str = Field(description="Claude's internal thinking process")
...
class ImageContent(BaseContentBlock):
    ...
    source: Dict[str, Any] = Field(description="Image source information")
...
class ToolUseContent(BaseContentBlock):
    ...
    id: str = Field(description="Tool use ID for linking to result")
    name: str = Field(
...
class ToolResultContent(BaseContentBlock):
    ...
    tool_use_id: str = Field(description="ID linking back to tool use request")
    content: Union[str, list] = Field(description="Tool execution output")
    is_error: Optional[bool] = Field(None, description="True if tool execution failed")


# Union type for all content blocks
ContentBlock = Union[
    TextContent, ThinkingContent, ImageContent, ToolUseContent, ToolResultContent
]
