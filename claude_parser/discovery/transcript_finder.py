...
import orjson
from toolz import filter as toolz_filter, map as toolz_map, compose, first
...
def _find_matching_project_dir(claude_projects: Path, target_path: Path) -> Optional[Path]:
    ...
    try:
        with open(jsonl_file, 'rb') as f:
            ...
            lines_with_cwd = toolz_filter(
                lambda line: b'cwd' in line and orjson.loads(line).get('cwd'),
                f
            )
