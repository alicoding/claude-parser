...
class CodebaseAnalyzer:
    ...
    def extract_file_inventory(self, filepath: Path) -> Dict[str, Any]:
        ...
        try:
            ...
            self.console.print(f"[yellow]Warning: Could not parse {filepath}: {e}[/yellow]")
            return None
    ...
    def analyze_codebase(self) -> Dict[str, Any]:
        ...
        excluded_dirs = {"__pycache__", ".git", ".pytest_cache", "venv", ".venv", "build", "dist"}
        python_files = [
        ...
        inventory = {
            ...
            "summary": {
                ...
                "total_functions": sum(len(inv["functions"]) for inv in all_inventories),
            },
            "packages": {}
        }
        ...
        for package, files in grouped.items():
            ...
            if package == "claude_parser":
                subgroups = groupby(lambda inv: self._get_subdomain(inv["file"]), files_list)
                ...
                for domain, domain_files in subgroups.items():
                    ...
                    inventory["packages"][package]["domains"][domain] = {
                        ...
                        "summary": {
                            ...
                            "class_count": sum(len(f["classes"]) for f in domain_files_list),
                            "function_count": sum(len(f["functions"]) for f in domain_files_list),
                        }
                    }
            else:
                inventory["packages"][package] = {
    ...
    for package_name, package_data in inventory["packages"].items():
        ...
        if "domains" in package_data:
            for domain_name, domain_data in package_data["domains"].items():
                domain_node = package_node.add(f"[green]{domain_name}/[/green] - {domain_data['responsibility']}")
                ...
                domain_node.add(f"[dim]{file_count} files, {class_count} classes, {func_count} functions[/dim]")
        else:
    ...
    for package_name, package_data in inventory["packages"].items():
        ...
        if "domains" in package_data:
            for domain_name, domain_data in package_data["domains"].items():
                ...
                lines.append(f"- **Functions**: {domain_data['summary']['function_count']}")
                lines.append("")
