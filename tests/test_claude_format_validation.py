...
class TestClaudeFormatValidation:
    ...
    def test_valid_claude_format_passes(self, tmp_path):
        ...
        assert is_valid is True, f"Valid Claude format should pass validation. Errors: {errors}"
    ...
    def test_non_claude_jsonl_fails_validation(self, tmp_path):
        ...
        assert any("Claude" in error for error in errors), f"Should mention Claude format in errors: {errors}"
    ...
    def test_load_with_strict_validation(self, tmp_path):
        ...
        assert "Claude Code format" in str(exc_info.value), "Should mention Claude Code format in error"
    ...
    def test_load_without_strict_validation_works(self, tmp_path):
        ...
        mixed_content = [
            '{"type": "user", "sessionId": "session-123", "message": {"content": [{"type": "text", "text": "Valid Claude"}]}}',
            '{"id": 1, "generic": "data"}',
            '{"type": "assistant", "sessionId": "session-123", "message": {"content": [{"type": "text", "text": "Another valid Claude"}]}}'
        ]
    ...
    def test_claude_format_signature_detection(self, tmp_path):
        ...
        assert is_valid is True, f"Should recognize Claude format signatures. Errors: {errors}"
...
class TestRealClaudeFileValidation:
    ...
    def test_real_claude_file_passes_validation(self):
        ...
        assert is_valid is True, f"Real Claude file should pass validation. Errors: {errors}"
        assert len(errors) == 0, f"Real Claude file should have no validation errors: {errors}"
        
        print(f"✅ Real Claude file validated successfully")