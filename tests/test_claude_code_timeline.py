...
class TestClaudeCodeTimeline:
    ...
    def _create_timeline_with_mock_data(self, project_path, jsonl_data):
        ...
        # Update file paths in jsonl_data to match the project_path
        updated_jsonl_data = []
        for event in jsonl_data:
            if event.get("type") == "assistant" and "message" in event:
                updated_event = event.copy()
                updated_message = event["message"].copy()
                updated_content = []
                for content_item in event["message"].get("content", []):
                    if content_item.get("type") == "tool_use" and "input" in content_item:
                        updated_content_item = content_item.copy()
                        updated_input = content_item["input"].copy()
                        if "file_path" in updated_input:
                            # Update file path to be relative to the actual project_path
                            original_path = updated_input["file_path"]
                            filename = Path(original_path).name
                            updated_input["file_path"] = str(project_path / filename)
                        updated_content_item["input"] = updated_input
                        updated_content.append(updated_content_item)
                    else:
                        updated_content.append(content_item)
                updated_message["content"] = updated_content
                updated_event["message"] = updated_message
                updated_jsonl_data.append(updated_event)
            else:
                updated_jsonl_data.append(event)

        with patch("claude_parser.domain.services.claude_code_timeline.jsonlines.open") as mock_jsonlines:
            # The timeline code calls list(reader), so we need to return an iterable
            mock_jsonlines.return_value.__enter__.return_value = iter(updated_jsonl_data)
    ...
    def test_initialization_with_mock_data(self, mock_claude_jsonl_data):
        ...
        with patch('claude_parser.domain.services.claude_code_timeline.find_all_transcripts_for_cwd') as mock_find:
            ...
            with patch('jsonlines.open') as mock_jsonlines:
                mock_jsonlines.return_value.__enter__.return_value = mock_claude_jsonl_data

                timeline = ClaudeCodeTimeline(project_path, mock_discovery)
    ...
    def test_tool_operation_extraction(self, mock_claude_jsonl_data):
        ...
        with patch('claude_parser.discovery.find_all_transcripts_for_cwd') as mock_find:
            mock_find.return_value = [Path("/mock/transcript.jsonl")]
    ...
    def test_chronological_ordering(self, mock_multi_session_data):
        ...
        with patch('claude_parser.discovery.find_all_transcripts_for_cwd') as mock_find:
            ...
            with patch('jsonlines.open') as mock_jsonlines:
                mock_jsonlines.return_value.__enter__.return_value = mock_multi_session_data
    ...
    def test_uuid_checkout(self, mock_claude_jsonl_data):
        ...
        with patch('claude_parser.discovery.find_all_transcripts_for_cwd') as mock_find:
            ...
            with patch('jsonlines.open') as mock_jsonlines:
                ...
                write_op = next(op for op in timeline.tool_operations if op.get('tool_name') == 'Write')
    ...
    def test_operation_diff_from_data(self, mock_claude_jsonl_data):
        ...
        with patch('claude_parser.discovery.find_all_transcripts_for_cwd') as mock_find:
            ...
            with patch('jsonlines.open') as mock_jsonlines:
                ...
                edit_op = next(op for op in timeline.tool_operations if op.get('tool_name') == 'Edit')
    ...
    def test_operation_diff_read_operation(self, mock_claude_jsonl_data):
        ...
        with patch('claude_parser.discovery.find_all_transcripts_for_cwd') as mock_find:
            ...
            with patch('jsonlines.open') as mock_jsonlines:
                ...
                read_op = next(op for op in timeline.tool_operations if op.get('tool_name') == 'Read')
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            jsonl_data = [
                {
                    ...
                    "message": {
                        "content": [{
                            "type": "tool_use",
                            "name": "Write",
                            "input": {"file_path": str(project_path / "app.py"), "content": "print('current project')"}
                        }]
                    },
                ...
                },
                {
                    ...
                    "message": {
                        "content": [{
                            "type": "tool_use",
                            "name": "Write",
                            "input": {"file_path": "/other/project/file.py", "content": "print('other project')"}
                        }]
                    },
            ...
            with patch('claude_parser.domain.services.claude_code_timeline.find_all_transcripts_for_cwd') as mock_find:
                ...
                with patch('jsonlines.open') as mock_jsonlines:
                    ...
                    assert timeline.tool_operations[0]["file_path"] == str(project_path / "app.py")
            ...
            with patch('jsonlines.open') as mock_jsonlines:
                ...
                for op in timeline.tool_operations:
                    if op.get('file_path'):
                        ...
                        assert op.get('file_path') == '/test/project/app.py'  # Original path preserved
        ...
        with patch('claude_parser.discovery.find_all_transcripts_for_cwd') as mock_find:
            ...
            with patch('jsonlines.open') as mock_jsonlines:
                ...
                with patch('builtins.print') as mock_print:
                    ...
                    with patch.object(ClaudeCodeTimeline, '_commit_operation') as mock_commit:
