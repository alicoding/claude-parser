...
class TestNavigationFeatures:
    ...
    def test_get_thread_navigation(self, real_conversation):
        ...
        for msg in conv.messages[10:20]:  # Check middle messages
            ...
            if len(thread) > 1:
                ...
                # Each message should reference the previous
                for i in range(1, len(thread)):
                    # This tests the threading logic
                    assert thread[i].parent_uuid is not None
                break
    ...
    def test_get_session_boundaries(self, real_conversation):
        ...
        if not hasattr(conv, 'get_session_boundaries'):
        ...
        # Each boundary should mark a sessionId change
        for boundary in boundaries:
            assert hasattr(boundary, 'index')
            assert hasattr(boundary, 'session_id')
...
class TestNavigationPerformance:
    ...
    def test_uuid_lookup_is_fast(self):
        ...
        uuids = [msg.uuid for msg in conv.messages[::10]]  # Every 10th message
