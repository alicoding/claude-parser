...
def test_core_imports_work():
    ...
    # Hook imports
    from claude_parser.hooks import (
        hook_input,
    )
...
def test_hook_api_structure():
    ...
    from claude_parser.hooks import HookData, exit_block, exit_success
...
def test_model_types_available():
    ...
    # Message is a Union type, others are classes
    ...
    assert get_origin(Message) is Union  # Message is a Union type
    assert isinstance(UserMessage, type)
    assert isinstance(AssistantMessage, type)
    assert isinstance(ToolUseContent, type)
    assert isinstance(ToolResultContent, type)
