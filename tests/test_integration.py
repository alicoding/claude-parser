...
from claude_parser import load, MessageType
...
class TestRealClaudeExport:
    ...
    @pytest.mark.skipif(not Path(REAL_JSONL_PATH).exists(), 
    ...
    def test_real_export_properties(self):
        ...
        print(f"✅ Properties work: {len(user_msgs)} user, {len(assistant_msgs)} assistant, {len(tools)} tools, {len(summaries)} summaries")
    ...
    def test_real_export_search(self):
        ...
        print(f"✅ Search works: {len(python_msgs)} python mentions, {len(error_msgs)} error messages")
...
class TestPerformance:
    ...
    def test_memory_usage_reasonable(self):
        ...
        assert memory_used < file_size * 5, f"Used {memory_used:.1f}MB for {file_size:.1f}MB file"
        
        print(f"✅ Memory: Used {memory_used:.1f}MB for {file_size:.1f}MB file ({len(conv)} messages)")
...
class TestEdgeCases:
    ...
    def test_empty_messages_handled(self, tmp_path):
        ...
        content = [
            '{"type":"user"}',  # Minimal user message
            '{"type":"assistant","content":""}',  # Empty content
            '{"type":"summary","summary":"","leafUuid":"123"}',  # Empty summary
        ]
        file.write_text('\n'.join(content))
        ...
        assert conv.summaries[0].text_content == ""
    
    def test_unicode_and_special_chars(self, tmp_path):
        """Test Unicode and special characters."""
        file = tmp_path / "unicode.jsonl"  
        content = [
            '{"type":"user","content":"Hello 👋 世界! How about math: ∑ ∞ ≈ π?"}',
            '{"type":"assistant","content":"I can handle émojis 🚀 and ñoñô characters!"}',
        ]
        file.write_text('\n'.join(content))
