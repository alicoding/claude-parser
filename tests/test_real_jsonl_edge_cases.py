...
class TestRealJSONLEdgeCases:
    ...
    def test_single_line_summary_only(self, minimal_sessions):
        ...
        if len(conv) == 1:
            msg = conv[0]
            assert msg.type in [MessageType.SUMMARY, MessageType.USER, MessageType.ASSISTANT]
    
    def test_medium_session_with_tools(self):
        """Test a medium-sized session with tool use."""
        file_path = Path.home() / ".claude/projects/-Volumes-AliDev-ai-projects-personal-assistant/1b1af05a-0cac-4e65-97df-6bb86368cbad.jsonl"
        ...
        assert MessageType.USER in message_types or MessageType.ASSISTANT in message_types
    ...
    def test_nested_content_blocks(self):
        ...
        test_files = [
            Path.home() / ".claude/projects/-Volumes-AliDev-ai-projects-claude-intelligence-center-hook-system-v2/8f64b245-7268-4ecd-9b90-34037f3c5b75.jsonl",
            Path.home() / ".claude/projects/-Volumes-AliDev-ai-projects-personal-assistant/0c8bb4c6-f1f8-4bd1-8cb8-773414fac973.jsonl",
        ]
    ...
    def test_session_boundaries(self):
        ...
        file_path = Path.home() / ".claude/projects/-Volumes-AliDev-ai-projects-memory/0c7808e6-8b5a-4e90-bf31-dffb7cc02da1.jsonl"
    ...
    def test_meta_messages(self):
        ...
        file_path = Path.home() / ".claude/projects/-Volumes-AliDev-ai-projects-claude-intelligence-center/0fa7629b-6811-4783-9bc2-cd84440c842d.jsonl"
        
        if not file_path.exists():
            # Try another file
            file_path = Path.home() / ".claude/projects/-Volumes-AliDev-ai-projects-personal-assistant/0c8bb4c6-f1f8-4bd1-8cb8-773414fac973.jsonl"
    ...
    def test_large_file_streaming(self):
        ...
        large_files = [
            Path.home() / ".claude/projects/-Volumes-AliDev-ai-projects-smart-prototyper/0128b69d-2ede-42e6-b715-87d8850a04d1.jsonl",  # 13MB
            Path.home() / ".claude/projects/-Volumes-AliDev-ai-projects-personal-assistant/0c8bb4c6-f1f8-4bd1-8cb8-773414fac973.jsonl",  # 3.6MB
        ]
    ...
    def test_validate_claude_format(self):
        ...
        claude_file = Path.home() / ".claude/projects/-Volumes-AliDev-ai-projects-personal-assistant/1b1af05a-0cac-4e65-97df-6bb86368cbad.jsonl"
...
class TestRealDataPatterns:
    ...
    def test_git_branch_tracking(self):
        ...
        file_path = Path.home() / ".claude/projects/-Volumes-AliDev-ai-projects-claude-intelligence-center-hook-system-v2/8f64b245-7268-4ecd-9b90-34037f3c5b75.jsonl"
        ...
        if conv.git_branch:
            ...
            # Common branch names
            assert conv.git_branch in ["main", "master", "develop", ""] or "/" in conv.git_branch
    ...
    def test_parent_uuid_threading(self):
        ...
        if len(messages) > 1:
            ...
            assert first_msg.get('parentUuid') is None or isinstance(first_msg.get('parentUuid'), str)
