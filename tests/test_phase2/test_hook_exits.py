...
def test_exit_success_with_message(capsys):
    ...
    # Message goes to stdout with newline
    captured = capsys.readouterr()
    assert captured.out == "Operation completed successfully\n"
    assert captured.err == ""
...
def test_exit_success_empty_string(capsys):
    ...
    # Empty string should not print anything
    captured = capsys.readouterr()
    assert captured.out == ""
    assert captured.err == ""
...
def test_exit_functions_type_hints():
    ...
    for func in [exit_success, exit_block, exit_error]:
        ...
        assert sig.return_annotation == NoReturn or sig.return_annotation == 'NoReturn', \
